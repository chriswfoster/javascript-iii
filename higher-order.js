...
let evenNumbers = mixedNumbers.filter(function(a) { return (a % 2) === 0})
...
let myStrongest = monstersInYourPocket.filter((filterMe) => filterMe.CP > 200)
...
let ordersTotal = orders.map((total) => total.price * (total.tax+1))
...
let allOfBobs = purchases.filter((filterage) => filterage.owner == 'Bob')
let bobsTotal = allOfBobs.reduce((total, addMe) => total + addMe.price, 0)
...
let bobsTotal = purchases
                                .filter(c=>c.owner == "Bob")
                                .map(c=>c.price)
                                .reduce((acc, cur) =>acc+cur, 0) // starts accumulator off at 0.
